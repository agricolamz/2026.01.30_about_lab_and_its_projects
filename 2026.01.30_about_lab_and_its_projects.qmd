...
- Чаще всего мы не собирали корпусов, а заключали договор с теми, кто его собирал, чтобы подготовить разметку:
...
- Ход таких исследований:
  ...
  - статистически моделировать вероятность появления нестандартной формы в зависимости от лингвистических и социолингвистических параметров
...
- в качестве основы надо брать нечто, что вариативно уже в литературном русском
    ...
    - интонация (кстати обратите внимание на проект [PRuD](https://lingconlab.github.io/PRuD/))
    - ...
- Но все равно объяснения будут носить разный характер:
  ...
  - билингвальная вариативность будет объясняться борьбой эффектов влияния L1 и неидеального освоения
...
Пороговое значение --- 10% от начала.

# Проекты по исследованию языков Кавказа

## Исследования абхазо-адыгских языков

- большие письменные корпуса
- маленькие устные диалектные корпуса
...
setwd("/home/agricolamz/work/scripts/r/markdown_EC_db")
library(tidyverse)
library(ragnar)

store <- ragnar_store_connect("east_caucasian_db")
...
ragnar_retrieve_vss(store,
  ...
  relocate(text, .before = "geneological_classification") |> 
...
store <- ragnar_store_connect("linguistic_abstracts")
...
- Корпусная лингвистика еще много может нам рассказать
  - хвала API НКРЯ! \pause
- Русский перевод открывает богатые возможности трансфера NLP инструментов для русского языка на корпуса и словари малоресурсных языков \pause
- LLM уже сидят на зубах, но хорошо бы не пропустить революцию
  - следует копить не только необработанные данные (звук, корпуса, словари), но и научные работы
